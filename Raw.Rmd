...
xtrain0 <- scale_movie[movie_sample==1, 1:5] # train set contaning predictors
xtest0 <- scale_movie[movie_sample==2, 1:5] # test set containing predictors



ytrain0 <- movie_num_all[movie_sample==1, 7] # train set containing the response "revenue"
ytest0 <- movie_num_all[movie_sample==2, 7] # test set containing the response "revenue"

# we can combine them into an overall data frame
train0 <- as.data.frame(cbind(profit = ytrain0,xtrain0)) 
test0 <- as.data.frame(cbind(profit = ytest0,xtest0))
train0$genres <- movie$genres[movie_sample == 1]
train0$company<- movie$company[movie_sample == 1]
train0$season <- movie$season[movie_sample == 1]
str(train0)
str(test0)
rev_lm10 <- lm(profit ~ budget + popularity + runtime + vote +genres + company, data = train0) # construct the model
summary(rev_lm10) # summary of the model
vif(rev_lm10) # check vifs
#movie_num_all[7]
...
```{r randomforestprofit}

set.seed(123)
rf_pro <- randomForest(profit ~., data = train0, ntree = 500, mtry = 3)
...
confusionMatrix(data = as.factor(as.integer(prof_glm_pred>0.5)), reference = test3$y)
```
