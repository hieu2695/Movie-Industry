...
```{r, echo = F, warning = F}
m_r <- subset(movie,select = c(company,month,genre,popularity,vote,score,budget,profit))
...
lasso_coef = predict(out, type = "coefficients", s = bestlam)[1:39,] # Display coefficients using λ chosen by CV
#lasso_coef
...
```{r}
str(movies_rf)
RFR = randomForest(x = subset(movie,select = c(budget,score,vote,runtime,popularity)),
                         y = movie$profit,
                         ntree = 500)
print(RFR)
```
...
### Logistic Regression
```{r logistic}
movie_l <- subset(movie,select = -c(title,date,genrecount,revenue,profit,roi))
movie_l$month <- as.factor(movie_l$month)
movie_l$runtime<-scale(movie_l$runtime)
movie_l$budget<-scale(movie_l$budget)
movie_l$popularity<-scale(movie_l$popularity)
movie_l$score<-scale(movie_l$score)
movie_l$vote<-scale(movie_l$vote)
str(movie_l)
