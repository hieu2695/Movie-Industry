...
Our goal is to answer the following SMART questions:
...
In this step, we rename the data, remove duplicates and missing values (there are few missing records so removing them does not hurt our analysis).
To evaluate the company variable, we divide the movie studios in 6 groups: Walt Disney, Warnes Bros, Sony, Universal, Paramount and Others which contains less famous studios. We also create new columns such as profit, profitable, season, quarter and year to support our research. All variables are updated to their correct formats (int, num, factor, date ...).
...
kable(summary(movie[c(6,1,3,7,9,10,11)]))
```
...
The p-values in the anova test with genre, company and season are `r summary(anov_genre)[[1]][["Pr(>F)"]][1]`, `r summary(anov_comp)[[1]][["Pr(>F)"]][1]` and `r summary(anov_ss)[[1]][["Pr(>F)"]][1]`, respectively. 
...
As seen from the results, score and runtime are not to be statistically significant. It seems that we can exclude these variables from the model. 
We will perform model selection using Adj R-squared, BIC and CP to optimize our model.
...
#show the metrics of test and train sets

#kable(as.data.frame(cbind(test = rev_lm_metrics1, train = rev_lm_metrics_train1)))
...
All predictors are statistically significant since their p-values are smaller than 0.05 level. 
The vifs are smaller than 3, which means that there the predictors are moderately correlated. The multicolliearity seems not to be problematic.
Adj R-squared is 0.711 which is the same as the model with full numerical predictors. We can say that roughly 71% of the variance found in
the response variable can be explained by the predictors.
...
kable(as.data.frame(cbind(test = rev_lm_metrics2, train = rev_lm_metrics_train2)))
...
The p-values and t-values indicate that there is no significance among different seasons and score is not statistically significant. Season and score seem not to be necessary predictors.
The overall effect of genre or company is significant.
...
When inlcuding the season, genre and company in the model, the best numerical predictors are not changed (budget, popularity and vote). The effects of different seasons seem to be the same. 
The best formula for the linear model in this case is: revenue ~ budget + vote + company + genres + popularity
...
kable(as.data.frame(cbind(test = rev_lm_metrics_test3, train = rev_lm_metrics_train3)))
...
kable(as.data.frame(cbind(AIC=AIC(object=rev_lm2), BIC=BIC(object=rev_lm2))))
```
...
kable(as.data.frame(cbind(AIC=AIC(object=rev_lm3), BIC=BIC(object=rev_lm3))))
...
rev_tree.rsq = 1 - printcp(rev_tree)[9,3] # we can calculate R-quared using equation: R-squared = 1 - rel error
                                          ...
                                          # I'm not sure if it is 100% correct, but I will assume this equation is correct and use it
...
kable(as.data.frame(cbind( CP = c(1:9), Error= rev_tree$cptable[,"xerror"]))) # we can see the lowest xerror
...
kable(as.data.frame(cbind(test = metrics_test_tree , train = metrics_train_tree )))

```

The R-squared in decision tree is `r 1-rev_tree$cptable[,"xerror"][8]` which is smaller than Adj R-squared in the linear model (0.711). The RMSEs and MAEs in decision tree are higher than those in linear model. Overall, the decision tree model is not as good as the linear model.
...
#kable(as.data.frame(randomForest::importance(rev_rf))) # the importance of each predictor
...
Using the tuneRF() function, we can find the best mtry. According to the graph, owest OOB Error is achieved at mtry = 4. 
We will buid a model with this mtry.
...
kable(as.data.frame(cbind(test = metrics_test_rf1  , train = metrics_train_rf1 )))
...
```{r str1}
#examine what we have in the dataset movies
str(movies)
```
...
#create columns for each individual month
M = factor(movies$month) 
dummm = as.data.frame(model.matrix(~M)[,-1])
movie_M = cbind(movie, dummm) 
#get a dataset only with profit and months
movies_c <- subset(movie_M, select = c(profit,M2,M3,M4,M5,M6,M7,M8,M9,M10,M11,M12))

#plot the rho with spearman
rho <- list()
month <- list()
for (i in 2:12){
  corr <- cor.test(x=movies_c[,i], y=movies_c$profit,method = 'spearman')
  rho[[i-1]] <- corr$estimate
  month[[i-1]] <- i
}
plot(month,rho,main = 'spearman')

#plot the rho with pearson
rho <- list()
month <- list()
for (i in 2:12){
  corr <- cor.test(x=movies_c[,i], y=movies_c$profit,method = 'pearson')
  rho[[i-1]] <- corr$estimate
  month[[i-1]] <- i
}
plot(month,rho,main = 'pearson')
```
...
#create a new dataframe with only profit and companies
...
company <- list()
for (i in 2:6){
  corr <- cor.test(x=movies_cor[,i], y=movies_cor$profit,method = 'spearman')
  rho[[i-1]] <- corr$estimate
  company[[i-1]] <- i
}
...
* By the result of our initial linear regression model on profit, we get that it is unlikely we will observe a relationship between some of the __predictors__ (like some uncommon genres) and the __response__ (profit)
...
* There are 38 coefficients for each model. The 201 $\lambda$ values are chosen from $10^{-10}$ to $10^{10}$, essentially covering the ordinary least square model ($\lambda$ = 0), and the null/constant 
...
Here, we see that the lowest MSE is when $\lambda$ = `r bl` and the r-squared becomes `r Lrsq`. The lasso regression drops 10 useless variables above and it has 27 non-zero coefficients. 
...
To predict the profit calculated by box office revenue minus budget, we take the ongoing FrozenII. With all the variales we have and those we take from its previous movie 'Frozen' like popularity, socre and vote, we get a estimation of the profit of FrozenII to be `r Frozen2_profita` using random forest regresssion.
...
With the polynomial regression, the estimated profit of movie FrozenII is `r Poly_predict`.  

# Chapter 5. PCA

Our goad in this chapter is to examine if Principal Component Analysis method is good at dimensional reduction. In our data, we have 5 continuous variables to predict the revenue. We perform principal component regression directly and see how many components are sufficient for the model.
...
All vifs are 1, which is a nice feature of PCA method to prevent multicollinearity. 
The scaled version gives better R-squared than non-scaled version, which is another indicator for why scaling data is recommended.
There is no big difference between the adj R-squared in linear model using 2 components (0.681) and the linear model using all variables (0.711). This shows that we can apply PCA to the linear model without hurting the predictive power.
...
The p-values for genres, company and season are `r format(wald.test(b = coef(prf_glm), Sigma = vcov(prf_glm), Terms = 7:23)[["result"]][["chi2"]][["P"]],  digits = 2)`, `r format(wald.test(b = coef(prf_glm), Sigma = vcov(prf_glm), Terms = 24:28)[["result"]][["chi2"]][["P"]], digits =2)` and `r format(wald.test(b = coef(prf_glm), Sigma = vcov(prf_glm), Terms = 29:31)[["result"]][["chi2"]][["P"]], digits = 2)`, respectively. It shows that the overall effect of genres is not statistically significant. The effect of company is the most significant.
...
All the companies are significant from each other. 
The Spring, Fall and Winter have the same effects, while Summer is significant from them.
...
The area under the curve is `r auc(h0)`, which also shows that the model is good enough. This test also agrees with the Hosmer and Lemeshow test.
...
negative profit = 0. (revenue < budget)
positive profit = 1. (revenue > budget)
...
The area under the curve is `r auc(h_dt)`. Classification tree seems not to be a good model in this case.
