...
rev_lm_train2 <- predict(object = rev_lm2, train1) # prediction on train set
...
train1_full$company <- movie[movie_sample==1, 1:14]$company # append the company to the test set
train1_full$season <- movie[movie_sample==1, 1:14]$season
...
rev_lm_test3 <- predict(object = rev_lm3, test1_full) # prediction on test set
...
rev_lm_train3 <- predict(object = rev_lm3, train1_full) # prediction on train set
...
prf_glm <- glm(y ~ ., data = movie_nd[-c(1)], family = "binomial")
summary(prf_glm)
...
#drop revenue
res.bestglm <- bestglm(Xy = movie_nd[-c(1)], family = binomial,
            IC = "AIC",                 # Information criteria for
            method = "exhaustive")
#summary(res.bestglm)
...
prf_glm0 <- glm(y ~ budget + popularity + score + vote + genres + company + season, data = movie_nd[-c(1)], family = "binomial")
summary(prf_glm0)
...
```{r coef}
lapply(exp(coef(prf_glm0))[c(23:27)], head) # exp coefficient of companies
#lapply(exp(coef(prf_glm0))[c(6:22)], head) # exp coefficient of genres
lapply(exp(coef(prf_glm0))[c(28:30)], head) # exp coefficient of genres
```
...
wald.test(b = coef(prf_glm0), Sigma = vcov(prf_glm0), Terms = 23:27)

wald.test(b = coef(prf_glm0), Sigma = vcov(prf_glm0), Terms = 6:22)

wald.test(b = coef(prf_glm0), Sigma = vcov(prf_glm0), Terms = 28:30)
```
...
prf_Hos = hoslem.test(movie_nd[-c(1)]$y, fitted(prf_glm)) # Hosmer and Lemeshow test, a chi-squared test
prf_Hos

# best model
prf_Hos0 = hoslem.test(movie_nd[-c(1)]$y, fitted(prf_glm0)) # Hosmer and Lemeshow test, a chi-squared test
prf_Hos0
...
prob_glm = predict(object = prf_glm, movie_nd[-c(1)], type = c("response")) # predict the model on train set
h <- roc(y~prob_glm, data=movie_nd[-c(1)])
...
prob_glm0 = predict(object = prf_glm0, movie_nd[-c(1)], type = c("response")) # predict the model on test set
h0 <- roc(y~prob_glm0, data=movie_nd[-c(1)])
...
```{r Q32.1}
#drop budget
res.bestglm1 <- bestglm(Xy = movie_nd[-c(2)], family = binomial,
            IC = "AIC",                 # Information criteria for
            method = "exhaustive")
#summary(res.bestglm)
...
prf_glm1 <- glm(y ~ revenue + popularity + score + vote + genres + runtime, data = movie_nd[-c(2)], family = "binomial")
summary(prf_glm1)
...
```{r Q32.6}
prob_glm1 = predict(object = prf_glm1, movie_nd[-c(2)], type = c("response")) # predict the model on train set
h1 <- roc(y~prob_glm1, data=movie_nd[-c(2)])
...
#confusion matrix
loadPkg("caret")
glm_pred <- predict(prf_glm, movie_nd[-c(1)], type = "response")

confusionMatrix(data = as.factor(as.integer(glm_pred>0.8)), reference = movie_nd[-c(1)]$y) # predicted Probability > 80%
#glm_pred
```


```{r sfs}
anova(prf_glm, test = 'Chisq')
...
movie_hw <- hw(movie.ts, initial = "optimal", h=23)
plot(movie_hw)
#accuracy(movie_hw) 
...
plot(forecast(movie_arima, h=23))
#movie_arima
...
highchart() %>% 
  hc_chart(type = "line") %>%
  hc_add_series_ts(movie.ts.test,name = "ACTUAL", color = "red" ) %>%
  hc_add_series(name = "PREDICTED", movie_ft, color = "blue" )  %>%
...
#plot(ts(movie_ts$revenue, frequency = 4, start = c(1995,1), end = c(2015,4)))
plot(movie_ft)
